...
static void mix_columns(uint8_t state[16]) {
    ...
    for (int i = 0; i < 4; i++) {
        int col = i * 4;
        temp[0] = gf_mul(0x02, state[col]) ^ gf_mul(0x03, state[col+1]) ^ 
                  state[col+2] ^ state[col+3];
        temp[1] = state[col] ^ gf_mul(0x02, state[col+1]) ^ 
                  gf_mul(0x03, state[col+2]) ^ state[col+3];
        temp[2] = state[col] ^ state[col+1] ^ 
                  gf_mul(0x02, state[col+2]) ^ gf_mul(0x03, state[col+3]);
        temp[3] = gf_mul(0x03, state[col]) ^ state[col+1] ^ 
...
static void inv_mix_columns(uint8_t state[16]) {
    ...
    for (int i = 0; i < 4; i++) {
        int col = i * 4;
        temp[0] = gf_mul(0x0e, state[col]) ^ gf_mul(0x0b, state[col+1]) ^ 
                  gf_mul(0x0d, state[col+2]) ^ gf_mul(0x09, state[col+3]);
        temp[1] = gf_mul(0x09, state[col]) ^ gf_mul(0x0e, state[col+1]) ^ 
                  gf_mul(0x0b, state[col+2]) ^ gf_mul(0x0d, state[col+3]);
        temp[2] = gf_mul(0x0d, state[col]) ^ gf_mul(0x09, state[col+1]) ^ 
                  gf_mul(0x0e, state[col+2]) ^ gf_mul(0x0b, state[col+3]);
        temp[3] = gf_mul(0x0b, state[col]) ^ gf_mul(0x0d, state[col+1]) ^ 
...
void gecko_aes_init(gecko_aes_ctx_t *ctx, const uint8_t key[32]) {
    ...
    ctx->num_rounds = 14;  /* AES-256 uses 14 rounds */
}
...
void gecko_aes_encrypt_block(const gecko_aes_ctx_t *ctx,
    ...
    sub_bytes(state);
    shift_rows(state);
    add_round_key(state, &ctx->round_keys[ctx->num_rounds * 4]);
